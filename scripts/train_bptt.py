#original source from Fenugreek/batteries: https://github.com/Fenugreek/batteries/blob/master/train.py
import numpy as np
import torch
import torch.nn as nn
import torch.nn.utils

def train_bptt(decoder, contexts, targets, seed,
    ...
    # drop batch and time, just get time t features shape
    # visible = torch.rand_like(contexts[:, -1, :, :, :]) 
    # hidden = decoder._init_hidden(batch_size)
    ...
    while marker < len(targets):
        ...
        for counter in range(bptt):
            ...
            outputs, hiddens = decoder(inputs)
